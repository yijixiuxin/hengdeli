...
class report extends base{
	public function __construct(){
		...
		if($user_info['level'] < 10 && $_GET['a']=='report1_2'){
		...
		//if(isset($_REQUEST['a']) && $_REQUEST['a']!='index'){
			...
			//$navArrays[] = array('title'=>"9.排行榜", 'datas'=>array(array('title'=>"9.1集团所有门店评分一览", 'url'=>"index.php?c=report&a=rank")));
			self::$tpl->assign('nav_arrays', $navArrays);
		//}else{
		//	self::$location[] = array('title'=>'高层阅读报告');
		//}
		if(strpos($_GET['a'],'report1')!==false){
			self::$location[] = array('title'=>'1.整体表现—集团与区域');
		}else{
			$gcode = $_GET['gcode'];
		...
		self::$tpl->assign('page_type', 'report');
	}
	...
	public function index(){
		...
		//$table_datas[] = array('title'=>"排行榜", 'datas'=>array(array('title'=>"集团所有门店评分一览", 'url'=>"index.php?c=report&a=rank")));
	...
	public function rank($dataonly=false){
		self::$location[] = array('title'=>'集团所有门店评分一览');
		...
		$table_titles = array('门店', '所在区域', '本期排名', '上期排名', '本期评分', '上期评分', '门店详情'); //th中的标题
		$table_datas = array();
		...
		foreach($finalData as $minfo){
			...
			$table_datas[] = array(
				...
				'ainfo'=>$ainfo,'rank'=>$minfo['rank'], 'prank'=>$minfo['prank'], 'defenlv'=>$minfo['defenlv'], 'pdefen'=>$minfo['pdefen'],
				"<a href='index.php?c=content&from=report&mcode={$minfo['info']['code']}'>查看</a>");
		}
		if($dataonly){
			return array('name'=>'集团所有门店评分一览','titles'=>$table_titles,'datas'=>$table_datas);
		}
		self::$tpl->assign('table_type', 'three_line');
		self::$tpl->assign('table_titles', $table_titles);
		self::$tpl->assign('table_datas', $table_datas);
		self::$page_style .= '<style type="text/css">tbody td.dtd_2, tbody td.dtd_4{font-weight:bold;}</style>';
	...
	private function navArrays($long=true){
		...
		if($level1){
			$temp = array('title'=>'1.整体表现—集团与区域', 'datas'=>array());
		...
		if($groups){
			foreach ($groups as $gcode => $ginfo) {
				...
				$table_datas[] = array(
					'title' => $tempTitle,
					'datas' => array(
						array('title'=>"{$tc}.1检测指标评分，及环比情况", 'url'=>"index.php?c=report&a=report2_1&gcode={$gcode}"),
						array('title'=>"{$tc}.2所有门店的评分分布", 'url'=>"index.php?c=report&a=report2_2&gcode={$gcode}"),
						array('title'=>"{$tc}.3本期不佳区域解析、上期不佳及达标区域追踪", 'url'=>"index.php?c=report&a=report2_3&gcode={$gcode}"),
					)
				);
				$tc ++;
			}
	...
	public function report1_1(){
		self::$location[] = array('title'=>'1.1集团总体评分、各区域评分，及环比情况');
		self::$tpl->assign('table_name', '1.1集团总体评分、各区域评分，及环比情况');
		$cstage = self::$context['cstage'];
	...
	public function report1_2(){
		self::$location[] = array('title'=>'1.2所有门店的评分分布');
		self::$tpl->assign('table_name', '1.2集团所有门店评分一览');
		$cstage = self::$context['cstage'];
		...
		$cAnalyse = model_statistics::analyseAll($cscores, $cfull);//var_dump($cAnalyse);die;
	...
	public function report1_3(){
		...
		$table_titles = array('区域名称'); //th中的标题
		$table_datas = array();
		...
		if($_REQUEST['ajax']=='chart'){
			$chartData = array(
				'chart'=>array('chart_name' => '各区域模块的得分率', 'xname'=>'区域名称', 'yname'=>'得分率'),
	...
	public function report1_6(){
		self::$location[] = array('title'=>'1.6本期表现不佳的区域解析(低于集团平均分)');
		$cstage = self::$context['cstage'];
	...
	public function report1_7(){
		self::$location[] = array('title'=>'1.7上期表现不佳的区域追踪(低于集团平均分)');
		self::$tpl->assign('table_name', '1.7上期表现不佳的区域追踪(低于集团平均分)');
		$cstage = self::$context['cstage'];
		...
		foreach ($areas as $acode => $ainfo) {
			...
			if($apscore < $pscore){
				...
				$data1[$acode] = array(
					...
					'rank' => 1,
					'score' => $acscore,
					'i' => calcIncrease($acscore,$apscore),
					);
				$certainAcode[] = $acode;
			}
		}
	...
	public function report1_8(){
		self::$location[] = array('title'=>'1.8上期达标区域追踪');
		$cstage = self::$context['cstage'];
		...
		foreach ($areas as $acode => $ainfo) {
			...
			if($ascore > 85){
				...
				$data1[$acode] = array(
					...
					'rank' => 1,
					'score' => $temppscore,
					'i' => calcIncrease($temppscore, $ascore), //($ascore=='-' || $temppscore=='-') ? '-' : number_format($temppscore-$ascore, 1)
					);
				$certainAcode[] = $acode;
			}
		}
	...
	public function report2_1(){
		...
		self::$location[] = array('title'=>"{$gcode}.1检测指标环比上期得分率");
		self::$tpl->assign('table_name', "{$gcode}.1检测指标环比上期得分率");
		$cstage = self::$context['cstage'];
	...
	public function report2_2(){
		...
		self::$tpl->assign('table_name', "{$gcode}.2全国门店该模块成绩分布、众数和中位数环比");
		$cstage = self::$context['cstage'];
	...
	public function report2_3(){
		...
		self::$location[] = array('title'=>"{$gcode}.3高层需重点关注的区域");
		...
		$tables = array(
			array('titles'=>array( '区域', '本期得分'), 'table_name'=>'本期表现不佳的区域成绩', 'datas'=>$data1)
		);
		...
		if( $pstage ){
			...
			$tables[] = array(
				'titles' => array('区域', '上期得分率', '本期得分率', '变化情况'),
				'table_name' => '上期表现不佳的区域',
				'datas' => $data2);
			$tables[] = array(
				'titles' => array('区域', '上期得分率', '本期得分率', '变化情况'),
				'table_name' => '上期达标区域本期表现',
				'datas' => $data3);
		}
		self::$tpl->assign('table_type', 'muilty_three_line');
		self::$tpl->assign('tables', $tables);
